...
class NavigationNode(Node):
    def __init__(self):
        ...
        self.inflation_radius = 1 # Radius inflasi obstacle, mungkin perlu disesuaikan
        self.PIXEL_TO_CM = 0.2 # Ini adalah estimasi, perlu dikalibrasi lebih lanjut
        self.faktor_kalib = 1.480 # Faktor kalibrasi tambahan
    ...
    def process_frame(self, img):
            ...
            if area > 50: # Minimal area kontur untuk dipertimbangkan
                ...
                if 0.7 < circularity < 1.1: 
                    ...
                    if M["m00"] != 0:
                        ...
                        #cv2.circle(img, target_pos, 5, (0, 255, 255), -1) # Titik tengah target
                        #cv2.drawContours(img, [cnt], -1, (0, 255, 0), 2) # Gambar kontur target yang terdeteksi
        ...
        if robot_pos and target_pos and robot_heading is not None:
            ...
            if inflated_grid[start_grid[0], start_grid[1]] == 0:
                self.get_logger().warn(f"Robot (start point {start_grid}) berada di area obstacle setelah inflasi. Mengabaikan A*.")
                self.send_zero_cmd_vel() # Berhenti jika robot di area obstacle
                return
            if inflated_grid[goal_grid[0], goal_grid[1]] == 0:
                self.get_logger().warn(f"Target (goal point {goal_grid}) berada di area obstacle setelah inflasi. Mengabaikan A*.")
                self.send_zero_cmd_vel() # Berhenti jika target di area obstacle
                return

            path = astar(inflated_grid, start_grid, goal_grid)
            smooth_path_result = smooth_path(inflated_grid, path)

            pixel_path = [to_pixel(p) for p in smooth_path_result]
            for i in range(len(pixel_path) - 1):
                cv2.line(img, pixel_path[i], pixel_path[i + 1], (255, 0, 255), 2)

            if len(pixel_path) >= 2:
                target_pixel_for_nav = pixel_path[1] # Titik selanjutnya di jalur yang diperhalus
            else:
                target_pixel_for_nav = target_pos # Jika jalur sangat pendek, langsung ke target
        ...
        # Tampilkan juga mask untuk debugging (opsional, bisa di-uncomment jika perlu)
        # cv2.imshow("Target Mask (Filtered)", mask_target)
        # cv2.imshow("Obstacle Mask", obstacle_mask)
        # cv2.imshow("Inflated Grid (Debug)", inflated * 255) # Tampilkan grid setelah inflasi

        if cv2.waitKey(1) & 0xFF == ord('q'):
            rclpy.shutdown()

    def send_zero_cmd_vel(self):
        # Tambahkan fungsi ini agar node sebelumnya bisa menerima perintah berhenti
        # jika robot atau target tidak terdeteksi
        msg_turn = Float32()
        msg_turn.data = 0.0 # Tidak berbelok
        self.turn_pub.publish(msg_turn)

        msg_dist = Float32()
        msg_dist.data = 9999.0 # Jarak sangat jauh, agar robot tidak memicu "stop_target"
        self.distance_pub.publish(msg_dist)
