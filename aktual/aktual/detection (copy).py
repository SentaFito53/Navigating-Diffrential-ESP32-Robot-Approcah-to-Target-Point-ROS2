    ...
    def process_frame(self):
        ...
        if ids is not None:
            ...
            for i, id_tag in enumerate(ids.flatten()):
                if id_tag == 17:
                    ...
                    #cv2.putText(img, f"ID: {id_tag}", (robot_pos[0] - 10, robot_pos[1] + 15),
        ...
        for cnt in contours:
            ...
            if area > 50:
                ...
                if M["m00"] != 0:
                    ...
                    cy = int(M["m01"] / M["m00"])
                    target_pos = (cx, cy)
                    cv2.circle(img, target_pos, 5, (0, 255, 255), -1)
                    break
        ...
        for gy in range(rows):
            for gx in range(cols):
                ...
                grid_cell = obstacle_mask[y_start:y_start + grid_px_h, x_start:x_start + grid_px_w]
        ...
        if robot_pos and target_pos and robot_heading is not None:
            start = to_grid(robot_pos)
            goal = to_grid(target_pos)
            ...
            if len(pixel_path) >= 2:
                target_pixel = pixel_path[1]
            else:
                target_pixel = target_pos

            dx = target_pixel[0] - robot_pos[0]
            dy = target_pixel[1] - robot_pos[1]
